...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':  # 1. Shift
            self.stack.append(self.buffer.pop(0))
        if len(self.stack) > 1:
            if transition == 'LA':  # 2. Left Arc
                self.dependencies.append((self.stack[-1], self.stack.pop(-2)))
            elif transition == 'RA':  # 3. Right Arc
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for parse, transition in zip(minibatch, model.predict(minibatch)):
...
def test_step(name, transition, stack, buf, deps,
    ...
    try:
        ...
        print("{:} test resulted in stack {:}, expected {:}".format(
            name, stack, ex_stack))
        passed = False
    ...
    try:
        ...
        print("{:} test resulted in dependency list {:}, expected {:}".format(
            name, deps, ex_deps))
        passed = False
...
def test_parse():
    ...
    try:
        ...
        print("parse test resulted in dependencies {:}, expected {:}".format(
...
if __name__ == '__main__':
    ...
    if len(args) != 2:
        raise Exception(
            "You did not provide a valid keyword. Either provide 'part_a' or 'part_b', when executing this script")
    elif args[1] == "part_a":
        test_parse_step()
        test_parse()
    elif args[1] == "part_b":
