...
# -*- coding: utf-8 -*-
"""
CS542 Spring 2021 Programming Assignment 5
...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        embed_to_hidden = torch.empty(
            self.n_features * self.embed_size + 1, self.hidden_size)
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.stack([self.embeddings[i] for i in w]).reshape(
            w.shape[0], self.n_features*self.embed_size)
        # END YOUR CODE
        return x
    ...
    def forward(self, w):
        ...
        x = torch.cat((self.embedding_lookup(w), ones), 1)
        h = torch.nn.ReLU()(torch.matmul(x, self.embed_to_hidden_weight))
        logits = torch.matmul(torch.cat((h, ones), 1),
